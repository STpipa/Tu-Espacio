...
- [ ] Probar el Paso B y el Paso C en celular con Expo Go (pinch-zoom de cámara; la URL de LAN ya se detecta sola, falta la prueba real con el teléfono) — **el paso más urgente para tenerlo "funcional" pronto**, requiere el celular del usuario a mano
- [ ] **Decisión del usuario:** ¿desplegar el server de Colyseus a Render.com (gratis, para que cualquiera se pueda unir sin estar en la misma wifi) o seguir en local por ahora mientras se sigue probando? (`server/src/index.ts` ya está listo para ese deploy tal cual)
