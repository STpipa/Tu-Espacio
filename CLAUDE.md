...
- **2026-07-28:** **Arranque de los puntos 1 a 6 pendientes** (el usuario pidió avanzar en los seis a la vez). Antes de tocar código en los que implican una decisión de producto o cuenta externa, se le preguntó con opciones concretas: eligió **Kenney (CC0, gratis, sin cuenta)** para los modelos de avatar, **dejar el audio real (WebRTC) para después** (es el cambio más grande de los seis, rompe Expo Go), y **Colyseus Cloud** para el hosting del server.
  ...
  - **Punto 4 (hosting Colyseus):** se agregó `.colyseus-cloud.json` al `.gitignore` de `server/` (Colyseus Cloud lo genera con credenciales en el primer deploy, no debe commitearse — mismo criterio que `.env`). El deploy en sí (crear cuenta, crear una "Application" en el dashboard de Colyseus Cloud, correr `npx @colyseus/cloud deploy` desde `server/`) queda pendiente de que el usuario cree la cuenta — no se puede hacer sin su login.
  - **Puntos 5 y 6 (RevenueCat real / reactivar Confirm Email):** sin cambios de código posibles sin que el usuario actúe primero — crear la cuenta de RevenueCat, y en el caso de Confirm Email es un toggle en el dashboard de Supabase (Authentication → Providers → Email) al que no se llega con la anon key.
...
- [ ] **Acción del usuario:** correr `supabase/schema_paso_f.sql` en el SQL Editor y avisar, para subir los 4 modelos `.glb` de Kenney y pegar el `UPDATE` con las URLs reales
...
- [ ] Probar el Paso B y el Paso C en celular con Expo Go (pinch-zoom de cámara; la URL de LAN ya se detecta sola, falta la prueba real con el teléfono)
- [ ] **Acción del usuario:** crear cuenta en Colyseus Cloud, crear una "Application" y correr `npx @colyseus/cloud deploy` desde `server/` — avisar cuando esté para actualizar `EXPO_PUBLIC_COLYSEUS_URL`
- [ ] **Acción del usuario:** reactivar "Confirm email" en Supabase antes de producción (Authentication → Providers → Email, hoy desactivado para pruebas)
...
- [ ] **Acción del usuario:** crear cuenta real de RevenueCat + instalar `react-native-purchases` cuando se quiera probar suscripciones de verdad (implica pasar a un dev client, ya no alcanza Expo Go/navegador) — ver `src/lib/revenuecat.ts`
